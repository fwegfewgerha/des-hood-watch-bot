...
- `ROBLOX_PLACE_ID` — the place ID of the Roblox game whose live player count sits in the bot's Discord presence (the number in a `roblox.com/games/<this>/name` URL). Optional and unrelated to the group vars — it uses only public endpoints, no key or cookie. Unset means the presence is left alone.
...
**Authorization**: the `Events.InteractionCreate` handler in `index.js` rejects any interaction whose `interaction.user.id` isn't in the `ALLOWED_USER_IDS` whitelist, before any command or component handler runs. This is a single global gate covering slash commands *and* the embed builder's components, and it is independent of (and stricter than) the per-command `setDefaultMemberPermissions(Administrator)` gating, which Discord alone enforces. The list itself lives in `utils/allowlist.js` (`isAllowedUser()`), because it is also the **exemption** list: a whitelisted user is skipped by automod (`handlers/autoMod.js`, before the spam tracker so their messages don't even accumulate history), by `/secure`'s invite deletion (`handlers/messageTracking.js`, by user ID rather than by permission, so it holds even when they post without a staff role on) and by the entire join path (`handlers/memberJoin.js` returns immediately, so they're never auto-kicked over account age and never counted toward the burst window or the raid alert). Everything else passive still applies to every member of the server: stat tracking keeps counting whitelisted users, since `/stats` is reporting, not moderation, and a channel lockdown is a per-channel permission overwrite with no per-user exemption — whitelisted operators keep talking through one only because they have Administrator.
...
- **CCU watch**: `utils/ccuWatch.js` keeps the bot's presence showing the live player count of `ROBLOX_PLACE_ID` ("Watching 12 players · DES HOOD"), started from `ClientReady` via `startCcuWatch()` next to `startBaselineTicker()`. Two public unauthenticated endpoints: a place maps to a universe once (`apis.roblox.com/universes/v1/places/<id>/universe`, cached for the process since it never changes) and the count comes from `games.roblox.com/v1/games?universeIds=<id>`. It polls once a minute but only pushes a presence update when the text actually changes, since Discord rate-limits presence far more tightly than this polls. Failures are logged once on the way down and once on recovery rather than once a minute, and the last known count is held through a short blip — but after 5 consecutive failures the number is dropped from the presence rather than keep asserting one that can no longer be confirmed.
