...
async function lockAllChannels(guild) {
  ...
  await store.setLockedChannels(guild.id, locked);
  return { locked, failed };
}

async function unlockAllChannels(guild) {
  const everyoneId = guild.roles.everyone.id;
  const lockedIds = await store.getLockedChannels(guild.id);
  ...
  await store.setLockedChannels(guild.id, []);
  await store.setRaidProtectActive(guild.id, false);
